  ...
  properties = list(
    ...
    DatasetType = bids_optional(
      ...
      setter = function(self, value) {
        ...
        stopifnot(value %in% c("raw", "derivative"))
...
bids_dataset_description <- function(
    ..., method = c("json_path", "json_string", "list"),
    .list = NULL) {
  ...
  if(method == "list") {
    ...
    if( method == "json_path" ) {
      json_path <- str[[1]]
      stopifnot(file_exists(json_path))
      json_path <- path_abs(json_path)
      json <- suppressWarnings({ readLines(json_path) })
    } else {
      json <- str
    }
  ...
  if(length(generated_by)) {
    dat$GeneratedBy <- drop_nulls(
      lapply(generated_by, function(kargs) {
        if(S7::S7_inherits(kargs, bids_generated_by)) {
          return(kargs)
        }
        if(length(kargs)) {
  ...
  re <- do.call(bids_dataset_description_impl, dat)
  attr(re, ".bids_source_path") <- json_path
  re
}
